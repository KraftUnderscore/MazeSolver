	...
	private void CreateNodes(Maze M) {
		for(int i=0; i<M.getHeight(); i++) {
			for(int j=0; j<M.getWidth(); j++) {
				if(M.getValue(j, i)!=-1) {
					Node n = new Node(M.getValue(j, i), j, i);
					G.addNode(n);
					int maxj = j+1;
					int maxi = i+1;
					...
					if(M.getValue(j, maxi)!=-1) {
						Node n1 = new Node(M.getValue(j, maxi), j, maxi);
						G.addNode(n1);
						G.addEdge(n, n1);
					}
					
					if(M.getValue(maxj, i)!=-1) {
						Node n2 = new Node(M.getValue(maxj, i), maxj, i);
						G.addNode(n2);
						G.addEdge(n, n2);
					}
